...
namespace DSXsharp
{
    public class DsxClient : APIbase, PublicAPI
    {
        ...
        public async Task<CurrencyPairs> GetCurrencyPairsAsync(CancellationToken cancellationToken)
        {
            try
            {
                string responseData = await base.RunQueryAsync("info", cancellationToken).ConfigureAwait(false);
        ...
        public async Task<OrderBook> GetOrderBookAsync(string pair, CancellationToken cancellationToken)
        {
            try
            {
                ...
                string responseData = await base.RunQueryAsync($"depth/{pair}", cancellationToken).ConfigureAwait(false);
        ...
        public async Task<Trades> GetTradesAsync(string pair, CancellationToken cancellationToken)
        {
            try
            {
                ...
                string responseData = await base.RunQueryAsync($"trades/{pair}", cancellationToken).ConfigureAwait(false);
        ...
        public async Task<Ticker> TickerAsync(string pair, CancellationToken cancellationToken)
        {
            try
            {
                ...
                string responseData = await base.RunQueryAsync($"ticker/{pair}", cancellationToken).ConfigureAwait(false);
        ...
        #region [ Priate API implementation ]
